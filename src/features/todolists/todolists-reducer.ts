...
const slice = createSlice({
  ...
  reducers: {
    ...
    setTodolists: (
      state,
      actions: PayloadAction<{ todos: TodoListsApiType[] }>,
    ) => {
...
>("todolists/delete", async (arg, thunkApi) => {
  ...
  try {
    dispatch(appActions.setAppStatus({ status: "loading" }));
    dispatch(
      todolistsActions.setEntityStatus({
        todoId: arg.todolistId,
...
export const addTodolist = createAsyncThunk<
  ...
  { title: string }
>("todolist/create", async (arg, thunkApi) => {
  const { dispatch, rejectWithValue } = thunkApi;
  try {
    dispatch(appActions.setAppStatus({ status: "loading" }));
    ...
    if (resp.data.resultCode === RESULT_CODE.SUCCEEDED) {
      dispatch(
        tasksActions.setTasksLists({
          todoID: resp.data.data.item.id,
          tasks: [],
        }),
      );
      dispatch(appActions.setAppStatus({ status: "succeeded" }));
